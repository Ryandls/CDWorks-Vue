<template>
  <div class="sidebar">
    ...
    >
      ...
      <div class="actions" v-if="isOwner"></div>
      <hr />
      <b-button @click="() => $router.push(`/profile/${user.id}`)" block
        >Meu Perfil</b-button
      >
      <b-button @click="() => $router.push(`/profile/${user.id}/skills`)" block
        >Habilidades</b-button
      >
      <b-button
        @click="() => $router.push(`/profile/${user.id}/portfolios`)"
        block
        >Trabalhos</b-button
      >
      <b-button @click="() => $router.push(`/profile/${user.id}/pic`)" block
        >Alterar Foto</b-button
      >
    </b-card>
  </div>
</template>
